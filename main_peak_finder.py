...
def cluster_groups_into_events(params, dataset, remove_singles=True, prefix=None, optimise_k=False,
    ...
    delta = 0.275
    ratio_matrix = np.exp(-ratio_matrix ** 2 / (2 * delta ** 2))#!!!!!!!!!!!!!!! or = 1 - ratio_matrix
    ...
    if optimise_k:
        ...
        best_std = 0
        best_k = 0
        for k in range(6, 11):  # could do (6, 21) -> min. 6 Configs through inspection + score worsens beyond 20
...
def events_summary(params, dataset, prefix=None, save_figs=False, avg_group=False):
    ...
    with open(event_path, 'rb') as loader:
        ...
        for label in np.unique(k_events.labels):
            ...
            print(s_samples)
            print(s_values)
            print(np.argwhere(k_events.labels == label).squeeze())
            ...
            # print(np.where(s_values <= 0)[0])
            # print(k_events.labels)
            # print(label)
            # print(s_values)
            # print(f'len(np.where(s_values <= 0)[0])={}')
            if len(np.where(s_values < 0)[0]) != 0: 
        ...
        for e in k_events.top_events[:, 0]:
            ...
            for ii, d in enumerate(indiv_diff):
                d_scaled = np.array(d) * np.array(s_weights)[ii]#array to float = mistake!!!!!!!!!!!!!!!!!!!!!!!
    ...
    with open(f'{path}/events.pkl', 'wb') as output:#!!!!!!!!!!!!!!!!! events
