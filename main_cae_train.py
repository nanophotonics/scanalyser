"""
@Created : 14/04/2021
@Edited  : 15/01/2023, 14/09/2023 (by Myron Sukhanov: train() enables multi-gpu training)
@Author  : Alex Poppe
@File    : cae_train.py
...
def train_cae_old(params):
    ...
    for epoch in range(init_epoch, params['c_epochs']):
        ...
        print(f"Time of Epoch {current_epoch} is {d_epoch}s. Train\Val Losses: {epoch_train_loss}, {epoch_valid_loss}")
...
def train_single_gpu(params, save_checkpoint_interval=3):
    """ Train and validate the CAE model using the BPT dataset
    ...
    # effective_batch_size = params['c_batch_size'] * strategy.num_replicas_in_sync
    effective_batch_size = params['c_batch_size']
    
    # Load in the validation dataset and its length
    valid_dataset, valid_len = load_dataset(params=params, data_type='valid', path='./data/valid_clean/valid_specs.csv')
    valid_dataset = valid_dataset.unbatch().shuffle(valid_len * 10).batch(effective_batch_size)

    train_dataset, train_len = load_dataset(params=params, data_type='train', path='./data/train_clean/train_specs.csv')
    train_dataset = train_dataset.unbatch().shuffle(train_len * 10).batch(effective_batch_size, drop_remainder=True)
    ...
    for epoch in range(init_epoch, params['c_epochs']):
        ...
        s_epoch = tf.timestamp()
        train_dataset_temp = train_dataset.unbatch().shuffle(train_len).batch(effective_batch_size, drop_remainder=True)
        d_load = tf.timestamp() - s_epoch

        current_epoch = epoch + 1
        print(f"---=== Epoch {current_epoch}/{params['c_epochs']} ===---")

        s_train = tf.timestamp()
        for train_data in train_dataset_temp:
            with tf.GradientTape() as tape:
                train_pred = model(train_data, training=True)
                loss = mse_loss(train_data, train_pred)
            gradients = tape.gradient(loss, model.trainable_variables)
            optimizer.apply_gradients(zip(gradients, model.trainable_variables))
            # loss = train_step(model, train_data, optimizer, mse_loss)
            epoch_train_loss += loss
        epoch_train_loss /= train_len  # Correct for dataset size differences
        train_losses.append(epoch_train_loss)
        ...
        if params['c_record']:
            with train_summary_writer.as_default():
                ...
                np.save(os.path.join(train_log_dir, 'train_losses.npy'), np.array(train_losses))
            # Save current weights and biases
            if tf.equal(current_epoch % save_checkpoint_interval, 0):
                encoder_mngr.save()
                decoder_mngr.save()
        ...
        for valid_data in valid_dataset:
            valid_pred = model(valid_data, training=False)
            epoch_valid_loss += mse_loss(valid_pred, valid_data)
        epoch_valid_loss /= valid_len  # Correct for dataset size differences
        valid_losses.append(epoch_valid_loss)
        ...
        print(f"Time of Epoch, Shuffling, Train, Val: {d_epoch:.2f}, {d_load:.2f}, {d_train:.2f}, {d_val:.2f}")
...
def train(params, checkpoint_interval=1, gpus_to_use=None):
    """ Train and validate the CAE model using the BPT dataset
    ...
    valid_dataset, valid_len = load_dataset(params=params, data_type='valid', path='./data/valid_clean/valid_specs.csv')
    valid_dataset = valid_dataset.with_options(options).unbatch().shuffle(valid_len * 100, seed=123456).batch(global_batch_size)

    train_dataset, train_len = load_dataset(params=params, data_type='train', path='./data/train_clean/train_specs.csv')
    train_dataset = train_dataset.with_options(options).unbatch().shuffle(train_len * 100, seed=123456).batch(global_batch_size, drop_remainder=True)
    ...
    with strategy.scope():
        ...
        encoder_mngr = tf.train.CheckpointManager(encoder_ckpt,
                                                    directory=f'./nn/checkpoints/cae/{params["c_ver"]}/encoder',
                                                    max_to_keep=400) #was 2

        decoder = model.layers[1]  # ...and the decoder, so that they can be run separately
        decoder_ckpt = tf.train.Checkpoint(optimizer=optimizer, model=decoder)
        decoder_mngr = tf.train.CheckpointManager(decoder_ckpt,
                                                    directory=f'./nn/checkpoints/cae/{params["c_ver"]}/decoder',
                                                    max_to_keep=400) #was 2, not 5
    ...
    for epoch in range(init_epoch, params['c_epochs']):
        ...
        for train_data in train_dist_dataset:
            ...
            # num_batches += 1
        epoch_train_loss /= train_len # * num_batches  # Correct for dataset size differences
        train_losses.append(epoch_train_loss)
        d_train = tf.timestamp() - s_train
        ...
        for valid_data in valid_dist_dataset:
            ...
            # num_batches += 1
        epoch_valid_loss /= valid_len # * num_batches  # Correct for dataset size differences
        valid_losses.append(epoch_valid_loss)
        d_val = tf.timestamp() - s_val
...
if __name__ == '__main__':
    ...
    train(params=hyperparams, checkpoint_interval=10, gpus_to_use = ["GPU:0"])
